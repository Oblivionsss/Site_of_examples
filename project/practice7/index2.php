<?php
    ...
    // Реализовать все с помощью классов
    ...
    // ПОтрачено более двух недель 
    // ПРиблизительно часов 20-30
    // Основная трудность - отсутствие мышления классами и ООП-но
    
    
    // v2.0
    // Сократить в каждом департаменте 40% (округляя в большую сторону) инженеров, преимущественно самого низкого ранга. 
    // Если инженер является боссом, вместо него надо уволить другого инженера, не босса. (не делал)
    // Увеличить в целях стимуляции умственной деятельности базовую ставку аналитика с 800 до 1100 тугриков, 
    // а количество выпиваемого им кофе с 50 до 75 литров. (вручную) В тех департаментах, где руководитель не является аналитиком, 
    // заменить его на аналитика самого высшего ранга из этого департамента (а бывшего руководителя вернуть к обычной работе) (вручную)
    ...
    // (сделал вручную).
    // Возможно вторую часть можно автоматизировать
    ...
    class pursh extends departament {                   // Определяем класс закупки с необходимы количеством сотрудников
        ...
        public function __construct() {                 // Инициализируем конструктором объектов-сотрудников
            $meneg9_1    = new meneger (4, 1, 0);       // 9 менеджеров 1 ранга
            $meneg3_2    = new meneger (6, 2, 0);   
            $meneg2_3    = new meneger (4, 3, 0);
    ...
    class sales extends departament {                   // класс - департамент продаж
        ...
        public function __construct() { 
            $meneg6_1  = new meneger (6, 1, 0);
            $meneg6_2  = new meneger (6, 2, 0);   
            $market6_1  = new market (6, 1, 0);  
            $market2_1  = new market (1, 2, 0);
            $analit3_1  = new analit (3, 1, 0);
            $analit2_2  = new analit (1, 2, 0);
            $departanalit2  = new analit (1, 2, 1);
            array_push($this->mas, $meneg6_1, $meneg6_2, $market6_1, $analit3_1, $analit2_2, $market2_1, $departanalit2); 
        }
    }   
    class adve extends departament {                    // класс - департамент рекламы
        ...
        public function __construct() {
            ...
            $meneg4_1    = new meneger (4, 1, 0);
            $meneg4_2    = new meneger (4, 2, 0);
            $ingen2_1    = new ingen (2, 1, 0);
            $departmarket3  = new market (1, 3, 1);
            array_push($this->mas, $market15_1, $market10_2, $meneg4_1, $meneg4_2, $ingen2_1, $departmarket3); 
        }
    }
    ...
    class logist extends departament {                  // класс - департамент логистики
        ...
        public function __construct() {
            $meneg6_1   = new meneger (6, 1, 0);
            $meneg7_2   = new meneger (7, 2, 0);   
            $meneg2_2   = new meneger (2, 2, 0);
            $meneg3_3   = new meneger (3, 3, 0);   
            $ingen5_1   = new ingen (5, 1, 0);
            $rdepartmmeneg1 = new meneger (1, 1, 1);
            array_push($this->mas, $meneg6_1, $meneg7_2, $meneg2_2, $meneg3_3, $ingen5_1, $rdepartmmeneg1); 
        }
    }


    abstract class Employee {
        public $coefANT     = 0.6;      // Антикризисные меры по инженерам
    ...
    class ingen extends Employee {
        ...
        public function __construct($number, $range, $checkBoss) {
            parent::__construct($number, $range, $checkBoss);
            if ($this->range == 1){ // Переопределяем количество инженеров
                $this->number   = round($this->number * $this->coefANT); //PHP_ROUND_HALF_UP
            }
        }
    }

    class analit extends Employee {
        protected $calary  = 1100;
        protected $cofee   = 75;
        protected $pages   = 5;
    }
?>